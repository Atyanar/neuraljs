...
(function(global) {
  ...
  var gaussRandom = function() {
    ...
    // ** 
    // returns a random value that is better than a pseudo-randomly generated value
    // **
    if(return_v) { 
      return_v = false;
      return v_val; 
    }
    // set to a random value between -1 and 1
    var u = 2*Math.random()-1;
    var v = 2*Math.random()-1;

    var r = u*u + v*v;
    // restart if r bigger than 1 or equal 0
    if(r == 0 || r > 1) return gaussRandom();
    
    //part of gaussian calculation
    var c = Math.sqrt(-2*Math.log(r)/r);
    v_val = v*c; // cache this
    return_v = true;
    // return u*c on first call, v*c on second one
    return u*c;
  }

  var randf = function(a, b) { return Math.random()*(b-a)+a; }
  var randi = function(a, b) { return Math.floor(Math.random()*(b-a)+a); }
  // random number median unit, standard deviation
